...
{ 
    ...
    //kernel->lock = new Lock("synch disk lock");
    hdrSector = sector;
}
...
if(seekPosition + numBytes > hdr->FileLength()) {           // need to expand
        ...
        freeMap->FetchFrom(kernel->fileSystem->freeMapFile);                   // fetch freemap
        ASSERT(hdr->Allocate(freeMap, numBytes));               // expand file
        hdr->WriteBack(hdrSector);                              // write header back
        freeMap->WriteBack(kernel->fileSystem->freeMapFile);                   // write freemap back
        delete freeMap;
    }
...
void
OpenFile::setFileName(std::string &fname)
{
    fileName = fname;
}
