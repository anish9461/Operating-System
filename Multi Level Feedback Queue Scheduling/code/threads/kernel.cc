...
int CompareCompletionTime(request *a, request *b)
{
    if(a->getCompletionTime() > b->getCompletionTime())
        return 1;
    else if(a->getCompletionTime() < b->getCompletionTime())
        return -1;
    else 
        return 0;
}
...
Kernel::Initialize()
{
    ...
    eventQueue = new SortedList<request*>(CompareCompletionTime); 
