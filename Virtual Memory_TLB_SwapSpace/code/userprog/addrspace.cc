...
AddrSpace::~AddrSpace()
{
   delete kernel->machine->pageTable;
}


int
AddrSpace::getNextSwapLoation()
{
    return swapCount++;
}
...
// how big is address space?
    ...
    cout << "Initializing address space for thread : " << kernel->currentThread->getName() << endl;
...
for(int i = 0; i < numPages; i++)
{
 //make an entry in map for swap location
   ...
   //read from executable into the buffer
    ...
    kernel->swapExecutable->WriteAt(buffer,PageSize,kernel->vpnToSwapMap[i] * PageSize);
...
AddrSpace::Execute() 
{
    ...
    cout << ">========================< Running User Program : " << kernel->currentThread->getName() << " >=====================<" << endl << endl;
...
AddrSpace::Translate(unsigned int vaddr, unsigned int *paddr, int isReadWrite)
{
    ...
    pte = &(kernel->machine->pageTable[vpn]);
