...
main(int argc, char **argv)
{
    ...
    cout << "Running User programs : " << endl;
    int i;
    char *debugArg = "";
    char *userProgName = NULL;        // default is not to execute a user prog
    List<char *> *userproglist = new List<char *>();
    ...
    for (i = 1; i < argc; i++) {
        ...
        userproglist->Append(userProgName);
	    i++;
	}
    ...
    while (!userproglist->IsEmpty()) {
        
        char * userprog = userproglist->RemoveFront();
        Thread *t = new Thread(userprog);
        t->Fork((VoidFunctionPtr)RunUserProg, (void *) userprog);
        cout << "    " << userprog << endl;
    }
    cout << endl;
    cout << "=========================== Number of Physical Pages = " << NumPhysPages << " ==========================" << endl << endl;
    delete userproglist;
